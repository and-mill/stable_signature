...
def decode_message_pil(pilimg, msg_extractor=None, device="cuda", verbose=True):
    transform_imnet = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406],std=[0.229, 0.224, 0.225])
    ])
    img = transform_imnet(pilimg).unsqueeze(0).to(device)
    return decode_message(img, msg_extractor=msg_extractor, device=device, verbose=verbose)
...
def eval_message(msg, verbose=True):
