...
def get_parser():
    ...
    aa("--train_dir", default="/home/host_datasets/COCO/train2017", type=str, help="Path to the training data directory")
    #aa("--val_dir", type=str, help="Path to the validation data directory", required=True)
    aa("--val_dir", default="/home/host_datasets/COCO/val2017", type=str, help="Path to the validation data directory")
    ...
    # ------------------------------------ params for fin --------------------------
    group = parser.add_argument_group('Params from FIN')
    aa('--diff', type=bool, default=False, help='different layer of input setting')
    aa('--message_length', type=int, default=64, help='message length of watermark')
    aa('--fed_path', type=str, default='FIN/experiments/JPEG/FED.pt', help='path of FIN cpkt path')
    # aa('--fed_linear_path', type=str, default='FIN/experiments/JPEG/')
    return parser
...
def main(params):
    ...
    # initialize wandb
    wandb.init(project='LDM_FIN_decoder', entity='wandb')
    # set the args as params to track
    ...
    # ----------------------------------------------------------------------------
    ldm_ae: LatentDiffusion = utils_FIN.load_model_from_config(config, params.ldm_ckpt)
    ...
    if 'torchscript' in params.msg_decoder_path: 
        ...
        # msg_decoder.eval()
        # ----------------------------------------------------------------------------
        msg_decoder = utils_FIN.get_hidden_decoder(num_bits=params.num_bits, redundancy=params.redundancy, num_blocks=params.decoder_depth, channels=params.decoder_channels).to(device)
        ckpt = utils_FIN.get_hidden_decoder_ckpt(params.msg_decoder_path)
        print(msg_decoder.load_state_dict(ckpt, strict=False))
        msg_decoder.eval()
        ...
        with torch.no_grad():
            ...
            for i, x in enumerate(loader):
                ...
                # ys.append(y.to('cpu'))
                # --------------------------------
                gaussian_noise = torch.zeros(x.shape[0], params.message_length).to(device)
                x = x.to(device)
                output_data = [x, gaussian_noise]
                im, y = msg_decoder(output_data, rev=True)
                ys.append(y.to('cpu'))
            ...
            # --------------------------- have problems to deal with the whitening ---------------------------
            # https://pytorch.org/tutorials/beginner/Intro_to_TorchScript_tutorial.html
            # torchscript_m = torch.jit.script(msg_decoder)
            # torchscript_m_linear = torch.jit.script(linear)
    
    msg_decoder.eval()
    # nbit = msg_decoder(torch.zeros(1, 3, 128, 128).to(device)).shape[-1]
    # ------------------------------------------------------------
    x_init = torch.zeros(1, 3, 128, 128).to(device)
    ms_init = torch.zeros(1, params.message_length).to(device)
    init_inputs = [x_init, ms_init]
    ims, ms_bits = msg_decoder(init_inputs, rev=True)
    nbit = ms_bits.shape[-1]
...
def train(data_loader: Iterable, optimizer: torch.optim.Optimizer, loss_w: Callable, loss_i: Callable, ldm_ae: AutoencoderKL, ldm_decoder:AutoencoderKL, msg_decoder: nn.Module, vqgan_to_imnet:nn.Module, key: torch.Tensor, params: argparse.Namespace):
    ...
    # --------------------------------------
    noise_layer = utils_FIN.JpegSS(params.batch_size)
    for ii, imgs in enumerate(metric_logger.log_every(data_loader, params.log_freq, header)):
        ...
        #    forward:   #
        #################
        input_data = [vqgan_to_imnet(imgs_w), keys]
        stego_img, left_noise = msg_decoder(input_data)
        ...
        gauss_noise = torch.zeros(left_noise.shape).cuda()
        output_data = [stego_noise_img, gauss_noise]
        re_img, decoded = msg_decoder(output_data, rev=True)
